...
def create_psd(currentwave, inventory):
    ...
    image_filename = 'ppsd.png' # replace with your own image
...
spectrogram_graph = dbc.Card([html.H3("Spectrogram"), dcc.Loading(
...
psd_graph = dbc.Card([html.H3("Power Spectral Density"), dcc.Loading(
    children=[html.Img(id = "psd")],
    type="circle")], body = True, style={"width": "50%"})
...
def update_seismometer_graph(start_date, end_date, dropdownvalue,
    ...
    results = create_waveform_graph(stations[dropdownvalue]["net"], dropdownvalue, stations[dropdownvalue]["chan"], start_date, end_date,
                                    filterselect, minfreq, maxfreq, cornersinp, zerophase)
    spectrogram = create_spectrogram(results[0], dbscaleinp)
    psd = create_psd(results[0], results[2])
    if not psd:
        return results[1], spectrogram, None
    return results[1], spectrogram, 'data:image/png;base64,{}'.format((psd).decode())
